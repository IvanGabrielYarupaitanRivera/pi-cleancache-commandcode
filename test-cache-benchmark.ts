 ...
 *
 * Usage:
 *   set COMMANDCODE_API_KEY=user_...
 *   npx tsx test-cache-benchmark.ts
 *
 * Sends N identical prompts and extracts cache metrics from the
 * assistant message in Pi's JSON output.
 */
...
const PROJECT_DIR = resolve(typeof __dirname !== 'undefined' ? __dirname : (typeof import.meta !== 'undefined' ? resolve(fileURLToPath(import.meta.url), "..") : process.cwd()));
...
/** Find the LAST assistant message with usage data in JSON-lines output */
...
function fmt(u: Usage): string {
  ...
  return (
    `↑${u.input.toLocaleString().padStart(6)}  ` +
    `↓${u.output.toLocaleString().padStart(6)}  ` +
    `R${u.cacheRead.toLocaleString().padStart(6)}  ` +
    `W${u.cacheWrite.toLocaleString().padStart(4)}  ` +
    `CH ${pct.padStart(5)}%`
  );
}
